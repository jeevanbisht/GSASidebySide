...
10. **Update README.md**: Add the new vendor to the "Available Guides" table and "Official Documentation" links:
    ```markdown
    | Vendor Name | [Source](VendorFolder/gsa-vendor-coexistence.html) | [🔗 Open Guide](https://jeevanbisht.github.io/GSASidebySide/VendorFolder/gsa-vendor-coexistence.html) |
    ```
11. **Update index.html**: Add a new vendor card to the landing page so the central hub stays current with all published guides.
    - Add a `.card-icon--vendor` CSS class with the vendor's brand color
    - Add an `<a class="guide-card">` block linking to the new guide HTML
    - Add a reference documentation `<li>` in the docs-list section
    - Update `grid-template-columns` if needed (e.g., `repeat(4, 1fr)` for 4 vendors)
12. **Publish to GitHub Pages**: Commit all changes and push to `origin/main`. The site auto-deploys via GitHub Pages at `https://jeevanbisht.github.io/GSASidebySide/`.
    ```powershell
    cd C:\AIProjects\jb\Inspire\SxSZ
    git add VendorFolder/ index.html README.md
    git commit --author="Jeevan Bisht <jeevanb@hotmail.com>" -m "Add GSA + Vendor coexistence guide

    - New guide with N scenarios based on official Microsoft Learn docs
    - Updated index.html with vendor card and reference link
    - Updated README.md with vendor entry

    Co-authored-by: Copilot <223556219+Copilot@users.noreply.github.com>"
    git push
    ```
    After push, verify the live page loads at `https://jeevanbisht.github.io/GSASidebySide/VendorFolder/gsa-vendor-coexistence.html`.
...
## Source Change Detection (Run on Every Invocation)

Before making any changes, **always check the official Microsoft Learn source pages** for updates. Compare the current content against what the guides were last built from.

### Source URLs to Check

| Vendor | Source URL |
|--------|-----------|
| Zscaler | https://learn.microsoft.com/en-us/entra/global-secure-access/how-to-zscaler-coexistence |
| Cisco Umbrella | https://learn.microsoft.com/en-us/entra/global-secure-access/how-to-cisco-coexistence |
| Palo Alto | https://learn.microsoft.com/en-us/entra/global-secure-access/how-to-palo-alto-coexistence |
| Netskope | https://learn.microsoft.com/en-us/entra/global-secure-access/how-to-netskope-coexistence |

### Workflow

1. **Fetch each source page** using `web_fetch` for all vendor URLs listed above
2. **Compare** the fetched content against the current guide implementation (scenarios, steps, config blocks, bypass FQDNs)
3. **Identify differences** — look for:
   - New or removed scenarios
   - Changed configuration values (IPs, FQDNs, ports)
   - Updated step instructions or ordering
   - New bypass endpoints or removed ones
   - Changed prerequisites or verification steps
4. **If changes are detected**: Report a summary to the user listing what changed per vendor, then **ask the user for instructions** before making any modifications
5. **If no changes detected**: Confirm sources are up-to-date and proceed with the user's original request
...
Zscaler (last checked: 2026-06-15)
  - 2 new bypass FQDNs added to Scenario 2 config
  - Step 4 instructions reworded

Palo Alto (last checked: 2026-06-15)
  - New Scenario 5 added: "GSA Internet Access only with Prisma for Private"
  - Tunnel IP range updated from 100.64.0.0/10 to 100.64.0.0/12

Cisco Umbrella — No changes detected.
